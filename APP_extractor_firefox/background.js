...
async function performExtractAndSave(initialUrl) {
    ...
    while (nextUrl) {
        try {
            ...
            const resultsRange = resultsRangeContainer
                .querySelector('h3')
                .textContent.replace('Results', '')
                .replace('records found', 'documents')
                .trim();
            ...
            const paragraphs =
                speechesDiv.querySelectorAll('.views-field-title');
            ...
            await Promise.all(
                urls.map(async (url) => {
                    try {
                        ...
                        const titleDiv = contentDoc.querySelector(
                            '.field-ds-doc-title'
                        );
                        ...
                        const authorElement =
                            contentDoc.querySelector('.diet-title');
                        ...
                        if (selectedFormat === 'xml') {
                            extension = '.xml';
                            fileContent = `<text author="${author}" title="${title
                                .replaceAll('&', '&amp;')
                                .replaceAll('"', '&quot;')}" date="${date}">
		<ref target="${url}">Link to original document</ref><lb></lb><lb></lb>${text.replaceAll(
                                '&',
                                '&amp;'
                            ).replaceAll('\n', '<lb></lb>')}</text>
					`;
